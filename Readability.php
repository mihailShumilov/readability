...
class Readability
{
    ...
    public function addFootnotes($articleContent)
    {
        ...
        for ($i = 0; $i < $articleLinks->length; $i++) {
            ...
            if ((strpos($articleLink->getAttribute('class'), 'readability-DoNotFootnote') !== false) || preg_match(
                    $this->regexps['skipFootnoteLink'],
                    $linkText
                )
            ) {
                continue;
            ...
            $footnoteLink->innerHTML = ($footnoteLink->getAttribute('title') != '' ? $footnoteLink->getAttribute(
                'title'
            ) : $linkText);
    ...
    function prepArticle($articleContent)
    {
        ...
        for ($i = $articleParagraphs->length - 1; $i >= 0; $i--) {
            ...
            if ($imgCount === 0 && $embedCount === 0 && $objectCount === 0 && $iframeCount === 0 && $this->getInnerText(
                    $articleParagraphs->item($i),
                    false
                ) == ''
            ) {
        ...
        try {
            ...
            $this->dbg(
                "Cleaning innerHTML of breaks failed. This is an IE strict-block-elements bug. Ignoring.: " . $e
            );
        }
    }
    ...
    protected function grabArticle($page = null)
    {
        ...
        for ($pt = 0; $pt < count($nodesToScore); $pt++) {
            ...
            if ($grandParentNode && !$grandParentNode->hasAttribute(
                    'readability'
                ) && isset($grandParentNode->tagName)
            ) {
        ...
        for ($c = 0, $cl = count($candidates); $c < $cl; $c++) {
            ...
            $this->dbg(
                'Candidate: ' . $candidates[$c]->tagName . ' (' . $candidates[$c]->getAttribute(
                    'class'
                ) . ':' . $candidates[$c]->getAttribute('id') . ') with score ' . $readability->value
            );
        ...
        for ($s = 0, $sl = $siblingNodes->length; $s < $sl; $s++) {
            ...
            $this->dbg(
                'Looking at sibling node: ' . $siblingNode->nodeName . (($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->hasAttribute(
                        'readability'
                    )) ? (' with score ' . $siblingNode->getAttribute('readability')) : '')
            );
            ...
            if ($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->getAttribute(
                    'class'
                ) == $topCandidate->getAttribute('class') && $topCandidate->getAttribute('class') != ''
            ) {
                $contentBonus += ((int)$topCandidate->getAttribute('readability')) * 0.2;
            }

            if ($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->hasAttribute(
                    'readability'
        ...
        for ($y = $targetList->length - 1; $y >= 0; $y--) {
            ...
            if ($isEmbed) {
                ...
                for ($i = 0, $il = $targetList->item($y)->attributes->length; $i < $il; $i++) {
                    $attributeValues .= $targetList->item($y)->attributes->item(
                            $i
                        )->value . '|'; // DOMAttr? (TODO: test)
                }
    ...
    public function cleanConditionally($e, $tag)
    {
        ...
        for ($i = $curTagsLength - 1; $i >= 0; $i--) {
            ...
            $contentScore = ($tagsList->item($i)->hasAttribute('readability')) ? (int)$tagsList->item($i)->getAttribute(
                'readability'
            ) : 0;

            $this->dbg(
                'Cleaning Conditionally ' . $tagsList->item($i)->tagName . ' (' . $tagsList->item($i)->getAttribute(
                    'class'
                ) . ':' . $tagsList->item($i)->getAttribute('id') . ')' . (($tagsList->item($i)->hasAttribute(
                    'readability'
                )) ? (' with score ' . $tagsList->item($i)->getAttribute('readability')) : '')
            );
                            ...
                            if ($li > $p && $tag != 'ul' && $tag != 'ol') {
                                ...
                                if ($input > floor($p / 3)) {
                                    ...
                                    if ($contentLength < 25 && ($embedCount === 0 && ($img === 0 || $img > 2))) {
                                        $this->dbg(
                                            ' content length less than 25 chars, 0 embeds and either 0 images or more than 2 images'
                                        );
                                        $toRemove = true;
                                    } else {
                                        if ($weight < 25 && $linkDensity > 0.2) {
                                            ...
                                            if ($a > 2 && ($weight >= 25 && $linkDensity > 0.5)) {
                                                $this->dbg(
                                                    ' more than 2 links and weight above 25 but link density greater than 0.5'
                                                );
                                                $toRemove = true;
                                            } else {
                                                if ($embedCount > 3) {
                            ...
                            if ($li > $p && $tag != 'ul' && $tag != 'ol') {
                                ...
                                if ($input > floor($p / 3)) {
                                    ...
                                    if ($contentLength < 25 && ($img === 0 || $img > 2)) {
                                        $this->dbg(
                                            ' content length less than 25 chars and 0 images, or more than 2 images'
                                        );
                                        $toRemove = true;
                                    } else {
                                        if ($weight < 25 && $linkDensity > 0.2) {
                                            ...
                                            if ($weight >= 25 && $linkDensity > 0.5) {
                                                ...
                                                if (($embedCount == 1 && $contentLength < 75) || $embedCount > 1) {
                                                    $this->dbg(
                                                        ' 1 embed and content length smaller than 75 chars, or more than one embed'
                                                    );
                                                    $toRemove = true;
                                                }
                                            }
